...
use constant PRIMITIVE_TYPES => [qw(
    string boolean null
...
use constant COMPOUND_TYPES => [qw(
...
sub new {
    ...
    my $self = bless {
        ...
        path        => $args{path}     // '',
        severity    => $args{severity} // JSON::Structure::Types::ValidationSeverity::ERROR,
        location    => $args{location} // JSON::Structure::Types::JsonLocation->unknown(),
...
sub to_string {
    ...
    push @parts, $self->{location}->to_string if $self->{location}->is_known;
    push @parts, "[$self->{code}]";
    push @parts, $self->{message};
    push @parts, "(schema: $self->{schema_path})" if defined $self->{schema_path};
...
sub make_validation_result { JSON::Structure::Types::ValidationResult->new(@_) }
sub make_validation_error  { JSON::Structure::Types::ValidationError->new(@_) }
sub make_json_location     { JSON::Structure::Types::JsonLocation->new(@_) }

# Severity constants
use constant SEVERITY_ERROR   => JSON::Structure::Types::ValidationSeverity::ERROR;
use constant SEVERITY_WARNING => JSON::Structure::Types::ValidationSeverity::WARNING;
